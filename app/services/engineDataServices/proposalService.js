...
} = require('zerotheft-node-utils/contracts/proposals')
    ...
    .process(async pid => {
      try {
        ...
        if (count > parseInt(lastPid) || isFailed) {
          ...
          const tmpYamlPath = `/tmp/main-${proposal.yamlBlock}.yaml`
          if (Object.keys(proposal).length > 0) {
            let proposalYaml
            try {
              proposalYaml = await proposalContract.callSmartContractGetFunc('getProposalYaml', [proposal.yamlBlock])
            } catch (e) {
              console.log(e)
            }
            ...
            fs.createReadStream(tmpYamlPath).pipe(
              fs.createWriteStream(`${proposalDir}/${propKey}_proposal-${proposal.date}.yaml`)
            )
...
const exportFailedProposals = async () => {
  ...
  try {
    const proposalIds = fs
      .readFileSync(failedProposalIDFile, 'utf8')
      .trim()
      .split('\n')
      .map(i => i)
