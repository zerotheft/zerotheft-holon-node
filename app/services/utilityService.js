...
const executeEnableCommand = async () => {
    ...
    return new Promise((resolve, reject) => {
        const env = config.MODE
        try {
            fs.writeFileSync('/etc/apt/sources.list.d/zerotheft-source.list', `deb [trusted=yes] https://zerotheft-holon-${env}.s3.us-east-1.amazonaws.com/releases stable main`)
