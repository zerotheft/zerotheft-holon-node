...
module.exports = async () => {
  ...
  if (!proxyStorage.address) {
    ...
    if (!storage.address) {
      error(
        chalk.red(
          'Please use zt-holon create-account to create your etc account. You can also use zt-holon import-account --private-key <privateKey>  to import your existing account '
        ),
        true
      )
    }
  ...
  if (answers.linkedin === true) {
    ...
    if (cookie) {
      try {
        ...
        console.log(
          chalk.blue(
            'Once you are logged out from your browser, this cookie wont work. Please make sure to update the cookie then.'
          )
        )
      } catch (e) {
...
const initialQuestions = async () =>
  inquirer.prompt([
    {
      name: 'maxVotes',
      message: 'Maximum votes a voter can vote by using this method:',
      type: 'input',
      default: 1,
      validate: i => {
    ...
    },
    {
      name: 'linkedin',
      message:
        "Do you want to verify by citizen's linked in profile? You need to give us cookie of the linkedin account.",
      type: 'confirm',
    },
  ])
...
const linkedinCookie = async () =>
  inquirer.prompt([
    {
      name: 'cookie',
      message: `Linkedin cookie file path: (If you don't know how to do that, we suggest to install EditThisCookie chrome extension, after installing open a logged LinkedIn website and via the extension click on export and paste it to a json file.)`,
      type: 'input',
      validate: async i => {
        try {
...
const linkedinValidations = async () =>
  inquirer.prompt([
    {
      name: 'connections',
      message: 'How many minimum connections should the profile have?',
      type: 'input',
      default: 5,
      validate: i => {
        if (isNaN(i)) return 'Please provide valid number'
