...
RELEASE_LOG=debinstall/releases/release-production.json
...
sed -i -E 's/(https:\/\/zerotheft-holon-)[a-z]+(.s3.us-east-1.amazonaws.com\/releases stable main)/\1production\2/g' $SOURCE_LIST_FILE
...
deb-s3 upload ${NEW_PKG} --prefix releases --bucket zerotheft-holon-production

echo -e "\033[32mStep 6: Deploy release log...\e[0m"
echo "{\"version\":\"${VERSION}\", \"date\":\"${TIME_STAMP}\"}"> $RELEASE_LOG
aws s3 cp $RELEASE_LOG   s3://zerotheft-holon-production/releases/ --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers