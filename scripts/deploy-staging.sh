...
RELEASE_LOG=debinstall/releases/release-staging.json
...
yarn --cwd ../zerotheft-holon-react build-staging

echo "setting up the environment as STAGING"
sed -i -E 's/( Environment=NODE_ENV=)[a-z]+/\1staging/g'  $SERVICE_FILE
sed -i -E 's/(https:\/\/zerotheft-holon-)[a-z]+(.s3.us-east-1.amazonaws.com\/releases stable main)/\1staging\2/g' $SOURCE_LIST_FILE

echo -e "\033[32mStep 3: Building debian package...\e[0m"
yarn pkg

cp debinstall/zerotheft-holon.deb ${NEW_PKG}
echo -e "\033[32mStep 4: File renamed...".${NEW_PKG}."\e[0m"

echo -e "\033[32mStep 5: Deploying in s3...\e[0m"
deb-s3 upload ${NEW_PKG} --prefix releases --bucket zerotheft-holon-staging

echo -e "\033[32mStep 6: Deploy release log...\e[0m"
echo "{\"version\":\"${VERSION}\", \"date\":\"${TIME_STAMP}\"}"> $RELEASE_LOG
aws s3 cp $RELEASE_LOG   s3://zerotheft-holon-staging/releases/ --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers